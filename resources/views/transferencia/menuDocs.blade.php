...
<form enctype="multipart/form-data" id="form_subeDocs" class="form-documentos form-horizontal form-solicitud" method="POST">
    ...
    <div class="panel panel-info panel-border top">
        <div class="panel-heading">
            <span class="panel-title">Ingreso de Solicitud N° {{$id}} - Adjuntar Documentación para la transferencia del vehículo</span>
        </div>
        ...
        <div class="panel-body">
            <div class="form-group">
                ...
                <label class="col-lg-2">
                    <input type="hidden" name="transferencia_rc_id" id="transferencia_rc_id" value="{{ isset($transferencia_rc[0]->id)? $transferencia_rc[0]->id :  0}}">
                    <input type="text" name="patente_ppu" id="patente_ppu" class="form-control" placeholder="XXX.999-2" value="{{ isset($transferencia_rc[0]->ppu)? str_replace(".","",explode("-",$transferencia_rc[0]->ppu)[0]) : ''}}" required disabled>
                    
                </label>
                <label class="col-lg-2"></label>
                
                <label for="transferencia_rc_num" class="col-lg-2 control-label">N° Solicitud RC :</label>
                <label class="col-lg-4">
                    <input type="text" name="nroSolRc" id="transferencia_rc_num" class="form-control" value="{{ isset($transferencia_rc[0]->numeroSol)? $transferencia_rc[0]->numeroSol :  0}}" required disabled>
                </label>
            </div>
...
$(document).on("click",".eliminarArchivoDoc2",function(e){
    ...
    $.ajax({
        url: "{{ route('transferencia.documento.destroy') }}",
        data: {
        ...
        success: function(data){
            $.ajax({
                    url: "/documentoTransferencia/"+solicitud_id+"/get",
                    type: "get",
...
$(document).on("submit","#form_subeDocs",function(e){
    ...
    $.ajax({
        url: "/documentoTransferencia/{{$id}}/cargadocs",
        type: "post",
        processData: false,
        contentType: false,
        data: formData,
