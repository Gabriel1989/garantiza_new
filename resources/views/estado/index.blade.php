...
<div class="panel panel-info panel-border top">
    <div class="panel-heading">
        <span class="panel-title">Listado de Estados de Solicitud</span>
        <div class="widget-menu pull-right">
            <a href="{{route('estado.create')}}" class="btn btn-info btn-sm"><li class="fa fa-plus"></li> Agregar Estado</a>
        </div>
    </div>
                    ...
                    <table id="tabla-data" class="table table-bordered">
                        ...
                        @foreach ($estados as $item)
                            <tr>
                                ...
                                <td>{{$item->name}}</td>
                                <td>
                                    <form action="{{route('estado.destroy', ['id' => $item->id])}}" class="form-eliminar" method="post">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="btn btn-system btn-sm" onclick="location.href='{{route('estado.edit', ['id' => $item->id])}}'">
